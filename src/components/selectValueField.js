...
const SelectValueField = ({ label, val=-1, options, enabled=true, onChangeFunction }) => {
    ...
    return (
        <Container>
            <Input id={`labelForSelectField_${uniqueId}`} className={(val !== '-1')?'entered':null} value={val} disabled={!enabled} onChange={onChangeFunction}>
                {(options.map((item, index) => <option key={`item_option_${label}_${uniqueId}_${index}`} value={item.id}>{item.value}</option> ))}
            </Input>
